---
title: "Praktikum 2 Analisis Regresi_Restu Ramadhani"
...
data <- read_excel("C:\\Downloads\\NSC-2025.xlsx", sheet="Sheet2")
...
Saat ini terdapat 1 peubah Y dan 9 peubah X, misal ingin dilakukan eksplorasi dan analisis terkait peubah X7
...
```{r}
b1 <- (sum(x*y)-sum(x)*sum(y)/n)/(sum(x^2)-(sum(x)^2/n)) #kalau mau menduga, stop sampai cari koefisiennya aja
b0 <- mean(y)-b1*mean(x)
...
dengan Y adalah persentase penduduk miskin dan X7 adalah indeks pembangunan gender.

Didapatkan nilai $\hat\beta_0 = -10.70937$ artinya nilai dugaan rataan indeks pembangunan gender ketika rata-rata lama sekolah bernilai 0 persen adalah -10.70937%, akan tetapi nilai 0 tidak ada dalam selang pengamatan rata-rata lama sekolah sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut.


Nilai $\hat\beta_1 = 0.9370334$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika Rata-rata lama sekolah berubah satu tahun adalah sebesar 0.9370334%

Koefisien determinasi yang dihasilkan adalah 0.5065923 atau 50.66% artinya keragaman persentase penduduk miskin yang dapat dijelaskan oleh persentase akses sanitasi layak adalah sebesar 50.66%, sisanya yaitu 49.34% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.
